...
class EE_Template {
	...
	public function parse(&$str, $is_embed = FALSE, $site_id = '', $is_layout = FALSE)
	{
		...
		if ($this->parse_php == TRUE AND $this->php_parse_location == 'output' AND $this->cache_status != 'CURRENT')
		{
			...
			$this->template = $this->parse_template_php($this->template);
                ...
                $this->parse_language($this->template);
	...
	// --------------------------------------------------------------------
        ...
        public function parse_language(){
                        
            try {				
                ee()->lang->loadfile($this->template_name,'template',false);
            }catch (Exception  $ex){
                error_log(print_r($ex,1));
            }
            foreach(ee()->lang->language as $key => $line){                
                $key = 'lang:'.$key;
                $this->template = str_replace(LD.$key.RD , $line, $this->template);
            }            
        }
	/**
	...
	public function parse_globals($str)
	{
		...
		{
			if (preg_match("/".LD."redirect\s*=\s*(\042|\047)([^\\1]*?)\\1\s*(status_code\s*=\s*(\042|\047)([^\\4]*?)\\4)?\s*(message\s*=\s*(\042|\047)([^\\7]*?)\\7)?".RD."/si", $str, $match))
			{
				...
				else
				{
					...
					$status_code = NULL;
                                        ...
                                        if ( isset($match[8]) ){
                                            $_SESSION['flash_message'] = $match[8];
                                        }
		...
		if ($query->num_rows() > 0)		{
            foreach ($query->result_array() as $row)                    {
                $global_vars[$row['variable_name']] = $row['variable_data'];
            }
		}
                //load global var from site files                
                $template_path =  ee()->config->item('tmpl_file_basepath') . '/' . ee()->config->item('site_short_name') . '/global_variables';                
                $glob_vars_files =  glob($template_path.'/*');
                foreach($glob_vars_files as $file){
                    $global_vars[basename( $file) ] = file_get_contents($file);
                }                                
                //load global var from template files                
                $template_path =  ee()->config->item('tmpl_file_basepath') . '/' . ee()->config->item('site_short_name') . '/'. $this->group_name.'.group/global_variables';                
                $glob_vars_files =  glob($template_path.'/*');
                foreach($glob_vars_files as $file){
                    $global_vars[basename( $file) ] = file_get_contents($file);
                }
                ...
                if (isset($_SESSION['flash_message'])){
                   $flash_message = explode('|', $_SESSION['flash_message'],2);
                   $global_vars['flash_message'] = $flash_message[0];
                   $global_vars['flash_class'] = @$flash_message[1];
                   unset($_SESSION['flash_message']);
                }
		
		if ( @isset($_SESSION['variables']) ){
		    foreach(@$_SESSION['variables'] as $name => $value) {
			$global_vars[$name] = $value;
		    }
		}


                foreach($global_vars as $variable_name => $variable_data){
                    $str = str_replace(LD.$variable_name.RD, $variable_data, $str);                    
                }
		...
		$str = str_replace(LD.'theme_folder_url'.RD,
			ee()->config->item('theme_folder_url'), $str);
                ...
                $str = str_replace(LD.'theme_url'.RD,
			 '/themes/site_themes/'. ee()->config->item('site_short_name') . '/'. $this->group_name .'.group/', $str);
	...
	public function advanced_conditionals($str)
	{
		...
		foreach ($user_vars as $user_var)
		{
			...
			$data['logged_in_'.$user_var] = ee()->session->userdata[$user_var];
                ...
                if (isset($_SESSION['flash_message'])){
                   $flash_message = explode('|', $_SESSION['flash_message'],2);
                   $data['flash_message'] = $flash_message[0];
                   $data['flash_class'] = $flash_message[1];
                   unset($_SESSION['flash_message']);
                }
	...
	// --------------------------------------------------------------------
        ...
        public function process_number($number, $format = '', $localize = TRUE){
	...
	public function process_date($timestamp, $parameters = array(), $relative = FALSE, $localize = TRUE)
	{
		...
		if ($relative)
		{
			...
			$localize = isset($parameters['timezone']) ? $parameters['timezone'] : $localize;
                        ...
                        if ( $parameters['format'][0] == '#' && is_numeric($timestamp) ){
                            $len = strlen($parameters['format']) ;
                            $commapos = strrpos($parameters['format'], ',' ) ;
                            $dotpos = strrpos($parameters['format'], '.' ) ;
                            $dec = $len -  $dotpos - 1 ;
                            $dec_point = '.';
                            $th_point = $commapos === FALSE  ? '' :',';
                            if ($commapos  > $dotpos) {
                                $dec =  $len - $commapos -1 ;
                                $dec_point = ',';
                                $th_point = '.';
                            }                            
                            $dt = number_format($timestamp,$dec,$dec_point,$th_point);
                        }else {                        
