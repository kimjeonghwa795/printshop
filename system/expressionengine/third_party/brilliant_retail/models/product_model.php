		...
		public function get_products($product_id = '' , $disabled = '',$cat='')
		{
			...
			if($product_id != ''){
				...
				// configuration file.
					...
					$disable_cache = (bool)($this->config->item('br_disable_product_cache'))  ;					
					if( ! $disable_cache){
...
//									$this->EE->TMPL->log_item('BrilliantRetail: return product_id ('.$product_id.') from cache file '.round(memory_get_usage()/1024/1024, 2).'MB');
			...
			//if ($product_id == 16 ) error_log($this->EE->db->_compile_select());
			...
			foreach ($query->result_array() as $row){
				...
				// General Product Details 
					$products[$i] = $row;
					//if ($product_id == 16 ) error_log(print_r($row,1));
				...
				// Product Attributes 
					...
					foreach($attributes as $attr){
						...
						$products[$i]['attributes'] .= '<p class="label">'.$attr['title'].'</p><p>'.$val.'<p>';
	 ...
	 */
    	public function get_product_collection($search,$limit=0,$offset=0,$sort,$dir,$cat_id='',$type_id='')
    	{
    		...
    		$prefix 	= $this->EE->db->dbprefix;
                ...
                $total= ee()->db->select('count(1) as cnt')->from('br_product')->get()->result_array()[0]['cnt'];
	 ...
	 */
    	public function search_products($term,$type=''){
    		...
    		$sql = ' SELECT * 
    		         FROM '.$this->EE->db->dbprefix.'br_product 
    		         WHERE enabled >= 0 AND site_id = '.$site_id.' 
                            AND (title LIKE "'.$term.'"
    		                 OR detail LIKE "'.$term.'"
    		                 OR sku LIKE "'.$term.'"
                                ) ';
	...
	*/
	...
	// Configurable Products				
		...
		// Set the quantity
				...
				$configurable[$i] = array(  'sku'             => $data['config_sku'][$i],
		...
		// Remove fields
			...
			// Setup our price matrix
				...
				// Create the data array for the post 
					$i = 0;					
					foreach($data['price'] as $i => $p){
						// We have an extra post for the template on the end
						// so only post length - 1
							//$i++;
							//error_log(print_r($p,1));
							//error_log(print_r(! $data[''],1));
							//error_log(print_r($data['price_qty'],1));
							//error_log(print_r($data['price_start'],1));
							//error_log(print_r($data['price_end'],1));
							...
							//if($i < (count($data['price']) - 1)){
								// Format the prices 
									...
									if($data['price_start'][$i] != ''){
										$d['start_dt'] = date('Y-m-d 00:00:00',strtotime($data['price_start'][$i]));
									}else{
										//$d['start_dt'] = '0000-00-00 00:00:00';
									}
									if($data['price_end'][$i] != ''){
										$d['end_dt'] = date('Y-m-d 23:59:59',strtotime($data['price_end'][$i]));
									}else{
										//$d['end_dt'] = '9999-13-00 00:00:00';
									}
						...
						// so only post length - 1
							if($i < (count($data['sale_price']) - 1)){
								...
								// Build the array
                                    ...
                                    if ($data['sale_price_end'][$i]){
                                        $d['end_dt'] = $data['sale_price_end'][$i];												
                                    }   
                                    if ($data['sale_price_start'][$i]){
                                        $d['start_dt'] = $data['sale_price_start'][$i];
									}
								$i++;
								$this->EE->db->insert('br_product_price',$d);
							}
			...
			$allowed_attr = array(	'site_id','type_id','title','url',
						'attribute_set_id','detail','detail_id','enabled','sku','shippable',
						'weight','featured','quantity','taxable',
						'cost','meta_title','meta_keyword','meta_descr','sort_order'
						);
			...
			// Update the product
					$attr['quantity'] = min($attr['quantity'],1000000);
					$attr['weight'] = floatval($attr['weight']);
					$this->EE->db->where('product_id',$product_id);
	 ...
	 * @return void
	 */
    	public function get_categories($enabled=1,$sort='sort',$parent_id='0'){
	 ...
	 * @return void
	 */
	public function get_category_meta($category_id=0){
		$cat = array();
		$this->EE->db->select('category_id,url_title,meta_title,meta_descr,meta_keyword');
		$this->EE->db->from('br_category');
                if ($category_id) $this->EE->db->where('category_id',$category_id);
		$query = $this->EE->db->get();
		foreach ($query->result_array() as $val){
			$cat[$val['category_id']] = $val;
		}
	 ...
	 * @return void
	 */
	public function get_category_collection($id,$featured=false){
            $products = array();
		
            if($id == '' && !$featured){return $products;}
		
            if(!isset($this->session->cache['br_get_category_collection'][$id])){
		// Get the category products
		$sql = "SELECT product_id 
			FROM exp_br_product_category 
                        WHERE category_id IN ($id) 
                        ORDER BY sort_order, product_id DESC";
		...
		$sql = 'SELECT  p.title, 
                                ...
                                p.sort_order,
                                p.featured 
                        FROM    exp_br_product p
                        WHERE   p.enabled = 1                             
                                AND p.product_id IN ('.join(',',$c).')';
                if ($featured) $sql .= ' AND p.featured ';
        ...
        // Start our product info array
                foreach($rst as $p){
                    $products[$p['product_id']] = array('title'         => $p['title'],
	...
	{
            $this->EE->db->delete('br_cart', array(	'session_id' => session_id(),'status' => 0));
	}
	...
	public function cart_get($cart_id = '')
	{
		...
		if(isset($this->session->cache['cart_get'][$session_id])){
			...
			$this->EE->db->from('br_cart');
                        if ($cart_id)
                            $this->EE->db->where('cart_id',$cart_id);
                        else 
	...
	public function get_cities($states = '',$full=false){
		$not_cached = array('0',1);
		$cache = $this->_get_cache('js_city_map', array());
				
		if(count($not_cached) > 1){
			$this->EE->db->select('value');
			$this->EE->db->where('code', 'jne_file');
			$query = $this->EE->db->get('br_config_data');			
			foreach($query->result() as $jne_config){$csv_file = unserialize($jne_config->value);}
		
			$csv = array_map('str_getcsv', file($csv_file['file']));			
			array_shift($csv);                    
			foreach ($csv as $i => $row){
				$cache[$row[1]][] = $row[2] ;
			}
			foreach($cache as $state => $cities){
				$cache[$state] = array_unique($cities);
			}
			$this->_set_cache('js_city_map', $cache);
		}
				
		return $cache;
	}
	public function get_districts($city = '',$full=false){
		$not_cached = array('0',1);
		$cache = $this->_get_cache('js_district_map', array());
		...
		if(count($not_cached) > 1){
			...
			foreach($cache as $c => $d){
				$cache[$c] = array_unique($d);
			}
			$this->_set_cache('js_district_map', $cache);
		}
		if ($city){
			return $cache[$city];
		}		
