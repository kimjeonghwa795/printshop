...
class Brilliant_retail extends Brilliant_retail_core{
    ...
    */
        function product($product_id=''){
            ...
            if(!$products){return $this->EE->TMPL->no_results();}
            ...
            // Form post url
                $action = $this->EE->functions->fetch_site_index(0,0).QUERY_MARKER.'ACT='.$this->EE->functions->fetch_action_id('Photoprint', 'product');
                ...
                if ($products[0]['type_id'] == 3){
                    //error_log(json_encode($products[0]["configurable_options"]));                    
                }
            // Parse Tags
                ...
                // parsion. Useful for adding tags, etc. ADDED 1.1.5 by dpd
					...
					//error_log($tagdata . PHP_EOL . $output);
                ...
                // Build the form and add the js to the footer
                    if($form == 'yes'){
						...
						$this->js('$(function(){
							$(".product-options select,.product-options input").css({"width":"auto","display":"inline"}).addClass("form-control cus-field product-options");
							function formatPrice(price){
								var symbol = "Rp ";
								return symbol + price.formatMoney(0) ;
							}
                            var configurable_options  = JSON.parse(\''. json_encode($products[0]['configurable_options']) . '\');
							$(".product-options select").change(function(){							
								var label = this.nodeName == "SELECT" ? this.options[this.selectedIndex].value : this.value;																								
								try {                                    
                                    var options = [];
                                    $("select.product-options").each(function(i,el){ options.push($(el).val());});                                    
                                    var config = configurable_options.find(function(opt){                                        
                                        return ! opt.options.map(function(o){return options.includes(o);}).includes(false);                                        
                                    });
									
									if (config){
                                        var baseprice = parseFloat($(".pricing").data("base_price"));                                     
                                        var newprice = parseFloat(config.adjust) + baseprice;
										$(".pricing p.price").html(formatPrice(newprice));
										$(".pricing p:first").html("SKU: " + config.sku  + " " );
                                        $("input[name=sku]").val(config.sku);
									}else{
                                        $(".pricing p:first").html( "Pilihan Anda belum lengkap");
                                    }
								}catch(err){												
									console.log(err);
									$(".pricing p:first").html("Start from ");
								}
								
								
							});
							$(\'#form_'.$products[0]["product_id"].'\').validate();
						});');
                    }else{
                        ...
                        if($show_js == 'yes'){
							...
							$(".product-options").change(function(){
								console.log(this.value, $(this).val() ,this );
							});
...
//			error_log($output . PHP_EOL . __LINE__ );
            return $output;
        }
    ...
    */
        ...
        public function catalog()
        {
            ...
            $key = $key ? $key : 'products';
            $featured    = (bool)$this->EE->TMPL->fetch_param('featured');
            $ajax        = $this->EE->TMPL->fetch_param('ajax','no');
            $form_class = $this->EE->TMPL->fetch_param('form_class','');
            ...
            if($qry->num_rows() != 1 && ! $featured ){
                return $this->EE->TMPL->no_results();
            }

            $products = $this->EE->product_model->get_category_collection($category[0]['category_id'],$featured );
                ...
                foreach($vars[0]["results"] as $row){
                    ...
                    $form_details = array(
                         'action'              => $action,
                          ...
                          'class'              => $form_class,
                          'hidden_fields'      => array('product_id' => $row["product_id"] ),
                          'secure'             => TRUE
                        
                      );
                    if (@$row['attr:next']) {
                        $form_details['hidden_fields']['next'] = @$row['attr:next'][0]['value'];
                    }
...
/********************/
        ...
        {
            $id = $this->EE->TMPL->fetch_param('id');
            $cart = $this->EE->product_model->cart_get($id);
...
//            if ( $this->EE->input->is_ajax_request() ){
//                $this->EE->output->send_ajax_response($count);
//                return 0;
//            }
        ...
        // Now add them
            foreach($post as $data){
                ...
                if($product[0]["type_id"] == 7){
                    ...
                    $amt["base"]  = $price;
                    $amt["price"] = $price;
                    $amt["price_html"] = '<p class="price" data-base_price="'.$price.'">'.$this->_format_money($price).'</p>';
        ...
        function checkout_form(){
            ...
            // Create the checkout form
                ...
                if($show_js === TRUE){
                    ...
                    $XID = CSRF_TOKEN;//$this->EE->security->generate_xid();
                    $countries = $this->EE->product_model->get_countries();
                    $states = $this->EE->product_model->get_states($countries);
                    $map = json_encode($states);                    
                    $cities = json_encode($this->EE->product_model->get_cities(current($states)));

                    // Add the JavaScript to the output cache
                        $checkout_js = "    var ship_to;
                                            var ship_note;
                                            var cnt = 0;
                                            var payment;
                                            var shipXHR;
                                            ...
                                            $(function(){
                                                ...
                                                $(document).on('change','.form-contact input,.form-contact select',function(){
                                            ...
                                            function _get_shipping_quote(address){
                                                ...
                                                shipXHR = $.post(url,params,function(data){
                                                        ...
                                                        $('#get_shipping_container').hide();
                                                        return _update_cart_totals();
                                                        $('#shipping_options select').remove();                        
                                                        var shippings= $('<div>' + data + '</div>').find('input:radio');
                                                        if (! shippings.length ) {
                                                            $('#shipping_options').html(data);
                                                        }else{
                                                            $('#shipping_options').append('<select name=\"shipping\" class=\"shipping  cus-field field-cont form-control width-xs\" required></select>');
                                                            $(shippings).each(function(i,el){
                                                                var prev_i = $(el);
                                                                var selected = prev_i.attr('checked') ? 'selected' : '';
                                                                prev_i.prop('checked',false).removeAttr('checked');
                                                                $('#shipping_options select').append('<option value=\"' + prev_i.val() + '\"' + selected + '>'+ prev_i.get(0).nextSibling.nodeValue.trim() +'</option>');
                                                            });
                                                        }                                                        
                                                        
                                                        $('#shipping_options div#options').hide();
                                            ...
                                            function _update_cart_totals(){
												...
												var prev_pay = $('input[name=gateway]:checked:eq(0)').id || 0;
                                                $.post(    url,
                                                            {
                                                                'XID' : '".$XID."',
                                                                'address1': $('input[name=br_'+address+'_address1]').val(),
                                                                'address2': $('input[name=br_'+address+'_address2]').val(),
                                                                'city':        $('input[name=br_'+address+'_city]').val() || $('select[name=br_'+address+'_city]').val() ,
                                                                'state':    $('#br_'+address+'_state').val(),
                                                                'zip':        $('input[name=br_'+address+'_zip]').val(),
                                                                'country':    $('#br_'+address+'_country').val(),
                                                                'shipping':    $('[name=shipping]:checked').val(),
																'coupon_code':$('[name=coupon_code]').val()
                                                            },
                                                            function(returndata){
...
//                                                                //console.log(data);
                                                                if(payment != data[0].payment){
                                                                    ...
                                                                    _bind_payment_options();
																	if (prev_pay){
																		$('.payment .gateways label[for='+prev_pay+']').click();
																	}else {
																		$('.payment .gateways label').first().click();
																	}
                                                                }
        ...
        function checkout(){
            ...
            // in case the form ommits one of the required fields
                ...
                $email = $this->EE->session->userdata["email"] ? $this->EE->session->userdata["email"] : $data['email'] ;
                ...
                if($member_id != 0) unset($required_fields['email']);
                ...
                // Added 1.6.2.4 - dpd 
                //error_log('validation before:' . $member_id .':');
                ...
                try {
                    ...
                    /*
                    if (!$ship_same_address && $data['br_shipping_country'] == 'ID')
                    {
            ...
            // End Shipping stuff            
            if($member_id){
                //error_log('check member_id :' . $member_id .':');
            ...
            // so we can send it to the payment gateway.
                ...
                //error_log('create_order_id' . $data['order_id']);
            ...
            // Create the order
                $order = array (
                    ...
                    "created"       => time(),
                    "updated"       => date('Y-m-d H:i:s' ,$this->EE->localize->now) ,
...
//======================== CREATE ORDER =================================================================================================
            ...
            // a shipment is necessary
                ...
                if(isset($_SESSION["shipping"][$data["shipping"]]["rate"])){
                    ...
                    $_SESSION["shipping"][$data["shipping"]]["status"] = 0;
                    $_SESSION["shipping"][$data["shipping"]]["tracknum"] = '';
                ...
                // Address
                    $address[0] = array(
                                        ...
                                        'billing_district'  => $data['br_billing_district'],
                                        ...
                                        'shipping_district' => $data['br_shipping_district'],
                ...
                // Add items to order
                    ...
                    foreach($data["cart"]["items"] as $items){
						...
						if (is_array($items['sku'])){$items['sku'] = $items['sku']['sku'];}

                        $item = array(	'order_id'        	=> $data['order_id'],
										'product_id'        => $items['product_id'],
										'configurable_id'   => $items['configurable_id'] ? $items['configurable_id'] : 0 ,
                ...
                // Finish up successful direct payments                    
                    if($order["status_id"] != -1){
                        // Send the email!
                            $vars[0] = array(	'fname'             => $data['br_fname'],
                                                'lname'             => $data['br_lname'],
                                                'email'             => $data['email'],
                                                'address'           => $address,
                                                'payment'           => $payment,
                        ...
                        // Redirect to thank you page with the order_id
                            $_SESSION['order_id'] = $data['order_id'];
                            $this->EE->functions->redirect($this->EE->functions->create_url($this->_config['store'][$this->site_id]['thankyou_url'])  .'/'. $data['order_id']);
                            exit();
                        ...
                        // Clear out the cache
                            ...
                            $data['ref']      = 'Order #' . $data['order_id'];
                            $data["phone"] = $data['br_billing_phone'];
        ...
        function checkout_shipping(){
            if(version_compare(APP_VER, '2.7.0', '>=')){$this->EE->security->restore_xid();}
            ...
            if($data["shippable"] == 0){
                ...
                $shipment = array(
                            ...
                            "to_city"        => $this->EE->input->post('city',TRUE),
                            "to_district"     => $this->EE->input->post('district',TRUE),
        ...
        function checkout_total()
        {
            ...
            $district    = $this->EE->input->post("district",TRUE);
            ...
            // Calculate Shipping
            //error_log(print_r($_SESSION["shipping"],1));
        ...
        function states(){
            ...
            // now get the states
				...
				$map = json_encode($states);				
				$cities = json_encode($this->EE->product_model->get_cities('',1));
				$districts = json_encode($this->EE->product_model->get_districts());
            ...
            // filter out countries without states
				...
				if ($placeholder) {
					array_unshift($states,$placeholder);
				}
                $this->EE->load->helper('form');
                $output = form_dropdown($name, $states, $value, $attr);				
				$checkout_js = '$(document).ready(function(){				                                                                                
                                        var country_state_map = '. $map .';
										var city_map = '. $cities .';
										var	district_map = '. $districts .' ;											

                                        $(document).on("change","[name$=state]",function(){
                                            var selectedState = $(this).val();
                                            var $city = $(this).parents(".form-contact").find("[name$=city]");       											
                                            var cities = city_map[selectedState] ;                                            																						
											var opts = "";											
                                            for ( var k in cities){												
                                               opts += (new Option(cities[k],cities[k])).outerHTML;
											   
                                            }; 							
											var attrs = { };
											$.each($city.get(0).attributes, function(idx, attr) {attrs[attr.nodeName] = attr.nodeValue;});

											$city.replaceWith($("<select>",attrs ).append(opts));
												
                                        });
										
										$(document).on("change","[name$=city]",function(){
											var selectedCity = $(this).val();											
											var $district = $(this).parents(".form-contact").find("[name$=district]");       
											
                                            var districts = district_map[selectedCity] ;                                            																						
											var opts = "";											
                                            for ( var k in districts){												
                                               opts += (new Option(districts[k],districts[k])).outerHTML;
											   
                                            }; 							
											var attrs = { };
											$.each($district.get(0).attributes, function(idx, attr) {attrs[attr.nodeName] = attr.nodeValue;});

											$district.replaceWith($("<select>",attrs ).append(opts));
										});
                                });';
		...
		function districts(){
            ...
            $districts = $this->EE->product_model->get_districts($city);
            $output =  '<select name="'.$name.'" id="'.$id.'" class="'.$class.'">';
        ...
        function state_city_map(){
            
        }
		
		function city_district_map(){		
            $districts = $this->EE->product_model->get_districts($city, (bool) $city);
