...
class Shipping_free extends Brilliant_retail_shipping {
	...
	function quote($data,$config){
		...
		$country = array('INA');
		$states  = array('DKI');
		if(!in_array($data["to_country"],$country)){return $this->rates;}	
		//if(!in_array($data["to_state"],$states)){return $this->rates;}	

		if($data["total"] >= $config["amount"]){
			$this->rates['free'] = array('code' => 'free','rate' => '0.00','label' => $config["label"]);
		}
