...
class phpVeritrans { 
	...
	// Array of the response variables from Veritrans requests
	public $Response;
	
	/* 	-------------------
		ERROR VARIABLES
		-------------------
		- $Error = an array of Veritrans's response to any paypal errors
		- All $_error's = are filled for any error that occurs, including Veritrans errors
		- If a method returns false, the $_error's should be filled with error information
	*/
	
	// Array of the error response from Veritrans - [TIMESTAMP] [CORRELATIONID] [ACK] [L_ERRORCODE0] [L_SHORTMESSAGE0] [L_LONGMESSAGE0] [L_SEVERITYCODE0] [VERSION] [BUILD]
	public $Error; 
	...
	*/
	...
	/*	-----------------------
		...
		- Veritrans does use the country CODES when sending address information; can come in very handy then.
		- List should include all countries that Veritrans accepts payments from.
	*/
	...
	/*	--------------------
		STATES
		--------------------
		- States for certain countries, many of these are required formats for Veritrans
	...
	*/
	...
	// CONSTRUCT - Updated per dpd to allow for configuration updates on
	// inital construct. 
	...
	public function do_capture()
	{
		...
		/* Construct the request string that will be sent to Veritrans.
		...
		/* Make the API call to Veritrans, using API signature.
	...
	{
		// TODO: Add error handling prior to trying to make Veritrans calls. ie: missing amount_total or RETURN_URL
		
		// urlencode the needed variables
		$this->urlencodeVariables();
		
		/* Construct the parameter string that describes the Veritrans payment
		...
		/* Make the call to Veritrans to set the Express Checkout token
			If the API call succeded, then redirect the buyer to Veritrans
		...
		/* Display the API response back to the browser.
		   If the response from Veritrans was a success, display the response parameters'
	...
	{
		// TODO: Add error handling prior to Veritrans calls. ie: missing TOKEN
		
		/* At this point, the buyer has completed in authorizing payment
			at Veritrans.  The script will now call Veritrans with the details
		 ...
		 /* Build a second API request to Veritrans, using the token as the
	...
	function do_express_checkout_payment()
	{
		...
		if(!empty($this->ItemsArray))
		{
		 ...
		 /* Make the call to Veritrans to finalize payment
	...
	/**
	  * hash_call: Function to perform the API call to Veritrans using API signature
	...
	*/
	private function hash_call($methodName, $nvpStr)
	{
	   ...
	   //Set proxy name to PROXY_HOST and port number to PROXY_PORT in constants.php 
		...
		if (curl_errno($ch)) 
		{
			...
			$this->_error_short_message	= 'There was an error trying to contact the Veritrans servers. (curl error) See long message for details.';
	...
	/* This function encodes all applicable variables for transport to Veritrans */
