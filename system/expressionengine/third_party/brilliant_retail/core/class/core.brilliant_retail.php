...
class Brilliant_retail_core {
		...
		// Get the BR _config variable & Set the site_id
			...
			// Check for local configuration of path / url settings
				$local_config_opts = array(	'br_media_url','br_secure_url','br_media_dir','br_license',
											...
											'br_downloads_s3_access_key','br_downloads_s3_secret_key','br_downloads_s3_length');
	...
	*/
	...
	function _format_money($amt){
            return $this->_config["currency_marker"]. ' ' . number_format($amt,0,'.',',');
	}
	...
	function _get_product($product_id){
			if($product_id == ''){
				...
				//error_log($product_id . PHP_EOL . $url_title);
				if($product_id != ''){
...
//                error_log(__LINE__. ':' . $products[0]['type_id']. PHP_EOL. print_r($products[0],1));
				if($products[0]["type_id"] == 3){
                    ...
                    $products[0]["configurable_options"] = $config_opts["options"];
				}else{
					$products[0]["configurable"][0] = array();
					$products[0]["configurable_js"] = '';
                    $products[0]["configurable_options"] = array();
				}
	...
	function _producttype_text($product_id,$attribute_id,$title,$label,$required,$val,$opts = ''){
			$class = ($required == 1) ? 'required product-options' : 'product-options' ;
		...
		function _producttype_file($product_id,$attribute_id,$title,$label,$required,$val,$opts = ''){
			...
			// Values  
                        $title 	= '';
                        $link 	= '';
                        $values = is_array($val) ? $val : unserialize($val);
                        $contain= '';
			...
			if(isset($values)){
				...
				if(trim(@$values["file"]) != ''){
					$link = '<span id="div_cAttribute_'.$attribute_id.'">
								<a href="'.$this->_config["media_url"].'file/'. str_replace($this->_config["media_dir"].'file/','',$values["file"]).'" target="_blank">'. basename($values["file"]).'</a>&nbsp;
...
//                    $sel 		= ($i == 0) ? 'checked="checked"' : '';
	...
	function _shipping_options($data){
		...
		foreach($this->_config["shipping"][$this->site_id] as $ship){
			// Is the method in the group list? 
                    ...
                    if(!in_array(0,$groups) && !in_array($group_id,$groups)){continue;  }
                    if($ship["enabled"] == 1){
                            $config = array();
		...
		foreach($rates as $r){
                            ...
                            foreach($r["quote"] as $q){
                                    ...
                                    $price = ($q["rate"]) ? $this->_format_money($q["rate"]) : '' ;
                                    $chk = ($i == 0) ? 'checked="checked"' : '';

                                    $vars[0]["rates"][] = array("rate"	=>   $price,
                                                                "rate_id"	=>   'shipping_'.$i, 
                                                                "rate_label"=>   $q["label"], 
                                                                "rate_value"=>   $hash,
                                                                "rate_checked"=> $chk
	...
	*/ 
		...
		if(isset($tmp)){$vars[0]["result_filter_set"] = $tmp;}
...
//                                                                if (! $b) continue;
	...
	function _build_config_opts($p)
	{
		...
		$pId = $p['product_id'];
		foreach($js['label'] as $i => $label){
			$options[$i] = array('label' => $label);
			foreach($js['rows'] as $p => $vals){
				
				foreach($vals['options'] as $key => $opt_id){
					$text_label = $js['option_label'][$opt_id]; 
					if 	( (float)$vals['adjust'] && $this->EE->config->item('br_config_hide_adjust_total') != 'y' ) {
						$sign =  abs($vals["adjust"]) == ($vals["adjust"]) ? '+ ' : '- ';
						$text_label .=  ' ('. $sign  . $this->_format_money(abs($vals["adjust"])) . ') ';
					}
					$options[$key]['options'][$opt_id] = $text_label;
				}
				
			}
			
			$select = '<select name="'. $pId .'_configurable_'.$i.'" id="'.$pId.'_configurable_'.$i.'" class="required"><option value="">'.lang('br_choose_an_option') . ' ' . $js['label'][$i] .'</option>';
			foreach ($options[$i]['options'] as $value => $text){			
			...
			//if($i == 0){
				...
				//$select = '<select name="'.$p["product_id"].'_configurable_'.$i.'" id="'.$p["product_id"].'_configurable_'.$i.'" class="required"><option value="">'.lang('br_choose_an_option') . ' ' . $js['label'][$i] .'</option>'.$opts.'</select>';
			//}else{
			//	$select = '<select name="'.$p["product_id"].'_configurable_'.$i.'" id="'.$p["product_id"].'_configurable_'.$i.'" class="required"><option value="">'.lang('br_choose_an_option') . ' ' . $js['label'][$i] .'</option></select>';
			//}
			*/
			$config[$i] =  array(
	...
	function _get_config_id($data,$product){
		...
		foreach($product["configurable"] as $c){			
			$options_ids = array();
			foreach($c['attribute'] as $atr){
					$options_ids[] = $atr['option_id'];
			}
			
			if (count(array_intersect($options_ids, $attr)) == count($attr)){							
				$this->EE->db->select('a.title, o.label');
				$this->EE->db->from('br_attribute a')->join('br_attribute_option o','a.attribute_id = o.attribute_id');
				$this->EE->db->where_in('o.attr_option_id',$options_ids);
				
				$rst = $this->EE->db->get();
				$options = array();
				foreach($rst->result_array() as $row){					
					$options[$row['title']] = $row['label'];
				}
				return  array('sku' => $c['sku'], 
	...
	/* Send Emails */	
        ...
        // Set some default variables
            ...
            if($this->EE->email->send()){
                $debug_msg = $this->EE->email->print_debugger(array());
                error_log($debug_msg);
                return true;
            }
		}
