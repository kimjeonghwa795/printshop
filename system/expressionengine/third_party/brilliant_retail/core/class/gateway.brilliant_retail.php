	...
	function ipn_create_order($merchant_id,$status)
	{
		...
		// There is an order waiting for IPN verfication
			...
			if($order["status_id"] == -1){
				...
				$vars[0] = array(   'fname' 			=> $order['member']['br_fname'],
				...
				$eml[0]['order_status'] = @$this->_config['status'][$order['status_id']];
	...
	private function _ipn_validate_order($merchant_id)
	{
		$this->EE->db->select('status_id,order_id')
                ...
                if (is_numeric($merchant_id))
                    $this->EE->db->where('order_id',$merchant_id);
                else
                    $this->EE->db->where('merchant_id',$merchant_id);
