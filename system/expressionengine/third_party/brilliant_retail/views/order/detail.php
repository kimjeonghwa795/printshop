...
<?php
    foreach($order['right'] as $row){
        foreach($row as $key => $val){
            echo '	<span class="button"><a id="'.strtolower(str_replace(" ","_",$key)).'"
                                title="'.$key.'" 
                                class="submit" 2x
                                target="'.$val['target'].'" 
                                href="'.$val['link'].'">'.$key.'</a>
...
<div id="popin-overlay" style="position: absolute;top: 0;left: 0;width: 100%;height: 100%;display:none;"></div>
	...
	<table width="100%" class="product_edit" cellpadding="0" cellspacing="0">
		...
		<tr>
			...
			<?php echo form_open($status_action,array('method' => 'POST', 'id' => 'statusForm'),$hidden); ?>
				<select name="status_id" id="status_id" onchange="if (this.value==4){document.querySelector('input.tracknum').style.display = 'block'; }else{document.querySelector('input.tracknum').style.display = 'none'; }">
				<?php 
				ksort($status);
				...
				<br />				
				<p>
                    <input type="checkbox" name="notify" checked="checke" value="1"/>&nbsp;<?=lang('br_status_notify')?>
				</p>
                <p>
                    <input type="text" name="tracknum" class="tracknum" placeholder="Tracking Number" value="<?=$order["shipment"][0]["tracknum"]?>" style="display:none"/>
                </p>
                <p>
                    <input type="text" name="order_note"  class="order_note" placeholder="Order Note" />
                </p>
			...
			<span id="order_status"><?=$status[$order["status_id"]]?> </span>
			</td>
		</tr>
		</table></td>
		</tr>
            	...
            	if(isset($order["items"])){
                	foreach($order["items"] as $item){
                            ...
                            echo '<tr class="'.$class.'">
                                    ...
                                    $array = explode("</li>", $options);
                                    $options = array();
                                    foreach ($array as $key => $val) {
                                        $key =  substr($val,strpos($val,'<label>') + 7 );
                                        $key =  substr($key,0,strpos($key,'</label>') );
                                        $val =  substr($val,strpos($val,'</label>') + 8 );
                                        $options[$key] = trim(strip_tags($val));
                                    }
                                    array_pop($options); // last element should be nothing

                                    $item_id = @$options['ItemId'];
//                                                echo '<div style="display:inline-block"><li>' . $options .'</ul></div>';
                                    echo '<div style="display:inline-block">';
                                    foreach($options as $k => $v){
                                        echo '<span><label> ' . $k .'</label>    ' .$v .'</span><br/>';
                                    }
                                    echo '</div>';
                                    $img_link = '/?ACT=76&order_id=' . $order['order_id'] . '&item_id=' . $item_id  . '&sku=' . $item['sku'] ;
                                    if (@$options['Title']){
                                        $img_link .= '&title=' . @$options['Title'];
                                    }
                                    echo '<a target="_blank" href="'.$img_link . '"><img src="'.$img_link.'&type=t&fmt=.png" class="photoprint preview" style="height:100px;float:right;"/></a>';
                                    if(trim($item["opts"]) != ''){
                                        echo $item["opts"].'<br />';
                                    }	
                                    echo '  </td>';
                                    echo '  <td>'.$item["sku"] .'</td>
                                            <td>'.$item["quantity"].'</td>
                                            <td colspan="2">'.$currency_marker.$item["price"].'</td>
                                    </tr>';
                	$i++;
                	}
		}
