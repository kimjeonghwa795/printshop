...
/**
 * ig_picpuller_lite Module Front End File
 *
 * @package		ExpressionEngine
 * @subpackage	Addons
 * @category	Module
...
class Ig_picpuller_lite {

	public $return_data;
	public $cache_name = PP_IG_PACKAGE;
	 ...
	 */
	 ...
	 public function beep()
	 {
		...
		$this->EE->TMPL->log_item('Pic Puller for Instagram: is installed and returning data. Beep.');
	 ...
	 public function userauthorized()
	 {
		...
		return $this->getInstagramId ($this->get_logged_in_user_id()) ? true : false;
         ...
         public function authorization_url(){
            ...
            return "https://instagram.com/oauth/authorize/?client_id=$clientID&redirect_uri=$redirect_uri&display=touch&response_type=code";
	 ...
	 public function authorization_link()
	 {
		...
		if (!$loggedInUser)
		{
			$this->EE->TMPL->log_item('Pic Puller for Instagram: Generating authorization for front end.');
                        $authorization_link = $this->authorization_url();
	...
	public function authorization()
	{
		...
		if (isset($user_data->{'access_token'})){
			...
			$this->EE->db->insert(PP_IG_PACKAGE.'_oauths');
                        $_SESSION['instagram_token'] = $user_data->{'access_token'};
		...
		switch ($response)
		{
			case 'success':
				//$this->showResult("Success", "You have authorized this site to access your Instagram photos.");
                                $_SESSION['flash_message'] = "You have authorized this site to access your Instagram photos.";
                                $_SESSION['token']['instagram'] = $user_data->{'access_token'};
                                $this->EE->functions->redirect('/instagram');
			break;
			case 'definedError':
	...
	private function getAuthCredsForUser($user_id)
	{
		...
		$this->EE->db->limit(1);
		$query = $this->EE->db->get(PP_IG_PACKAGE.'_oauths');
	...
	{
             $action_id = ee()->functions->fetch_action_id(__CLASS__, 'authorization');
             return ee()->config->item('site_url') .'/?ACT='  . $action_id ;
	}
