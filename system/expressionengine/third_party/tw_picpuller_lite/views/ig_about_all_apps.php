...
<table border="1" cellspacing="2" cellpadding="5" width='100%'>
	...
	for ($i = 0; $i < count($client_ids); $i++) {
		...
		if ($ig_picpuller_lite_prefixs[$i] === ''){
			$ig_picpuller_lite_prefixs[$i] = '<em>No prefix set.</em>';
		}

	?>
	<tr ><td><?php if ($site_ids[$i] === $current_site_id) { echo"<strong>";} ?><?=$site_labels[$i];?><?php if ($site_ids[$i] === $current_site_id) { echo"</strong>";} ?><td><?=$client_ids[$i];?></td><td><?=$client_secrets[$i];?></td><td><?=$ig_picpuller_lite_prefixs[$i];?></td></tr>
	<?php } ?>
</table>
<br>
