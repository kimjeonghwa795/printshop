...
class Ig_picpuller_lite_mcp {
	...
	public function index()
	{
		...
		$vars['moduleTitle'] = lang(PP_IG_PACKAGE.'_module_name');
		$vars['moduleShortTitle'] = lang(PP_IG_PACKAGE.'_short_module_name');
		...
		$action_id = $this->EE->functions->fetch_site_index(0, 0).QUERY_MARKER.'ACT='.$this->EE->cp->fetch_action_id(PP_IG_PACKAGE, 'authorization');
		...
		*/
		...
		else
		{
			...
			$vars['action_url'] = 'C=addons_modules'.AMP.'M=show_module_cp'.AMP.'module='.PP_IG_PACKAGE.AMP.'method=save_settings';
		}
	...
	public function ig_info()
	{
		...
		$vars['frontend_auth_url'] = $this->getFrontEndAuth();
		$vars[PP_IG_PACKAGE.'_prefix'] = $this->get_ig_picpuller_prefix();
	...
	{
		$this->EE->db->select('site_id,app_id,ig_client_id,ig_client_secret,site_label,ig_picpuller_lite_prefix');
		$this->EE->db->from(PP_IG_PACKAGE.'_credentials');
		$this->EE->db->join('sites', PP_IG_PACKAGE.'_credentials.ig_site_id = sites.site_id');
		...
		$ig_picpuller_lite_prefixs = array();
		...
		foreach ($query->result() as $row)
		{
			...
			array_push($ig_picpuller_lite_prefixs, $row->ig_picpuller_lite_prefix);
		}
		...
		$vars[PP_IG_PACKAGE.'_prefixs'] = $ig_picpuller_lite_prefixs;
		$vars['moduleTitle'] = lang(PP_IG_PACKAGE.'_module_name');
		$vars['moduleShortTitle'] = lang(PP_IG_PACKAGE.'_short_module_name');
	...
	public function ig_users()
	{
		...
		if ($this-> appExistsInDb() ) {
			$vars['appexists'] =  TRUE;
			$this->EE->db->select(PP_IG_PACKAGE.'_oauths.member_id, screen_name, oauth');
			$this->EE->db->where('app_id', $this->_currentAppId );
			$this->EE->db->from(PP_IG_PACKAGE.'_oauths');

			$this->EE->db->join('members', PP_IG_PACKAGE.'_oauths.member_id = members.member_id');
			$query = $this->EE->db->get();
	...
	public function edit_secret_step2()
	{
		...
		$this->EE->db->update(PP_IG_PACKAGE.'_credentials', $data);
	...
	public function edit_prefix_step2()
	{
		...
		$ig_client_id = $this->getClientID();
		$ig_picpuller_prefix = $this->EE->input->post(PP_IG_PACKAGE.'_prefix', TRUE);
		$data = array(
			PP_IG_PACKAGE.'_prefix' => $ig_picpuller_prefix
		);
		...
		$vars['client_secret'] = $this->getSecret();
		$vars['frontend_auth_url'] = $this->getFrontEndAuth();
		$vars[PP_IG_PACKAGE.'_prefix'] =$ig_picpuller_prefix;
		$vars['homeurl'] = $this->_base_url;
		$vars['ig_info_name'] =  $this->EE->lang->line('ig_info');
		$vars['cancel_url'] = $this->_base_url.'&method=ig_info';
		return $this->EE->load->view('update_settings_confirmation', $vars, TRUE);
	...
	public function delete_app()
	{
		...
		if ( $this->isSuperAdmin() ) {
			...
			$this->EE->db->delete(PP_IG_PACKAGE.'_credentials', array('app_id' => $appID));
			$this->EE->db->delete(PP_IG_PACKAGE.'_oauths', array('app_id' => $appID));
	...
	public function removeAuthorization()
	{
		...
		$this->EE->db->delete(PP_IG_PACKAGE.'_oauths');
	...
	public function adv_user_admin()
	{
		...
		$this->EE->db->from(PP_IG_PACKAGE.'_oauths');
		$this->EE->db->where(PP_IG_PACKAGE.'_oauths.app_id', $this->_currentAppId );
		$this->EE->db->where('member_id', $this->getLoggedInUserId());
		$this->EE->db->limit(1);
		$query = $this->EE->db->get();
	...
	public function save_user_info_adv() {
		...
		$this->EE->db->insert(PP_IG_PACKAGE.'_oauths');
	...
	{
		$this->EE->db->select(PP_IG_PACKAGE.'_prefix');
		$this->EE->db->where('ig_site_id', $this->_currentSite);
		$this->EE->db->limit(1);
		$query = $this->EE->db->get(PP_IG_PACKAGE.'_credentials');
		...
		foreach ($query->result() as $row)
		{
			$ig_picpuller_prefix = $row->ig_picpuller_lite_prefix;
		}
	...
	private function appAuthorized()
	{
		...
		$this->EE->db->where('member_id', $this->getLoggedInUserId() );
		$this->EE->db->join(PP_IG_PACKAGE.'_credentials', PP_IG_PACKAGE.'_credentials.app_id = '.PP_IG_PACKAGE.'_oauths.app_id');
		$this->EE->db->where(PP_IG_PACKAGE.'_credentials.ig_site_id', $this->_currentSite);
		$this->EE->db->limit('1');
		$this->EE->db->from(PP_IG_PACKAGE.'_oauths');
		$query = $this->EE->db->get();
