...
class Wizard extends CI_Controller {
	...
	var $next_update		= TRUE;	// The next update version that needs to be loaded, when an update is performed.
	...
	function _preflight()
	{
		...
		$cache_path = EE_APPPATH.'cache';
                error_log('cache_path' .  $cache_path);
                error_log(ee()->config->item('cache_path'));
		...
		if ( ! $this->_db_connect($db))
		{
			...
			return FALSE;
                ...
                error_log('CONFIGURATION' . print_r($config,1));
		// EXCEPTIONS
